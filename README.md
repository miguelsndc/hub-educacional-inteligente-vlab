...
Esta plataforma resolve esse problema centralizando o gerenciamento e reduzindo o esforço de catalogação através de **Assistência Inteligente**, um auxílio pedagógico baseado em IA que sugere descrições e tags a partir do título e tipo do material. O princípio fundamental da é a IA atuar como um co-piloto, não como um oráculo. Todas as sugestões são editáveis, e o sistema foi feito para operar perfeitamente mesmo sem a intervenção ou disponibilidade da inteligência artificial.
...
---
**Nota sobre o modelo**: Durante o desenvolvimento, o uso de versões fixas da API do Gemini (como 1.5 ou 2.0) resultava em erros de cota excedida (o 429). A adoção do alias *gemini-flash-latest* foi o contorno adotado para garantir a estabilidade das requisições.
...
A resposta da IA é validada via Pydantic de forma rigorosa antes de chegar ao frontend. Caso o modelo alucine o formato (um comportamento inerente à LLM's), o backend intercepta a falha e retorna um erro amigável. O estado do formulário é preservado, permitindo que o usuário tente novamente ou assuma o preenchimento manual.
